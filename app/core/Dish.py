...
def create_dish_for_restaurant(restaurant_name: str, dish: DishSchema.DishCreate, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
...
def get_dishes_by_type(food_type: search_enums.FoodTypeEnum, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
...
def get_dishes_by_diet(diet_type: search_enums.DietEnum, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
...
def get_dishes_by_cuisine(cuisine: search_enums.CuisineEnum, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
...
def get_dishes_by_cuisine(establishment: search_enums.EstablishmentTypeEnum, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
...
@router.get("/all", 
            ...
            status_code=status.HTTP_200_OK)
def get_all_dishes_endpoint(db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
...
@router.get("/featured_dish", 
            ...
            status_code=status.HTTP_200_OK)
def get_featured_dish(update: bool = False, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
